...
def test_images(client, noauth):
    ...
    """
    # Skip login when authentication has not been implemented
    if not noauth:
        # Log in
...
def test_links(client, noauth):
    ...
    forms = [x.get("action") for x in soup.find_all("form")]
    ...
    assert ("/likes/" in forms) or ("/likes/?target=/" in forms)
    assert ("/comments/" in forms) or ("/comments/?target=/" in forms)
    ...
    assert "uncomment" not in buttons
...
def test_timestamps(client, noauth):
...
def test_comments(client, noauth):
