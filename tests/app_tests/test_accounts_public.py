...
def test_accounts_authorization(client):
    """Verify the /accounts/auth/ route.

    Note: 'client' is a fixture fuction that provides a Flask test server
    interface with a clean database.  It is implemented in conftest.py and
    reused by many tests.  Docs: https://docs.pytest.org/en/latest/fixture.html
    """
    # Make a request to /accounts/auth/. The status code should be 403, because
    # we're not logged in.
    response = client.get("/accounts/auth/")
    assert response.status_code == 403
    ...
    # Make another request to /accounts/auth/. Now the status code should be
    # 200.
    response = client.get("/accounts/auth/")
    assert response.status_code == 200
    ...
    # Make one more request to /accounts/auth/. The status code should be 403
    # again.
    response = client.get("/accounts/auth/")
    assert response.status_code == 403
